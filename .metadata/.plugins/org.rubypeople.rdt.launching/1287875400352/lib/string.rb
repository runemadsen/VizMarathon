...
------------------------------------------------------------------------
     ...
     transform names for different purposes. For instance, you can
     figure out the name of a database from the name of a class.
...
------------------------------------------------------------------------
     Extension for String class. This feature is included in Ruby 1.9 or
     later but not occur TypeError.

     String#% method which accept "named argument". The translator can
     know the meaning of the msgids using "named argument" instead of
     %s/%d style.
...
------------------------------------------------------------------------
     ...
     Why would we ever use Ruby 1.8.7 when we can backport with
...
Includes:
---------
     Comparable(<, <=, ==, >, >=, between?), Diff::LCS(diff, lcs, patch,
     patch!, sdiff, traverse_balanced, traverse_sequences, unpatch,
     unpatch!), Enumerable(all?, any?, as_json, collect, count, cycle,
     detect, drop, drop_while, each_cons, each_slice, each_with_index,
...
----------
     ...
     INTERPOLATION_PATTERN:             Regexp.union(      
                                        /%\{(\w+)\}/,                   
                                                   # matches
                                        placeholders like "%{foo}"      
                                        /%<(\w+)>(.*?\d*\.?\d*[bBdiouxXe
                                        EfgGcps])/  # matches
                                        placeholders like "%<foo>.d"
     INTERPOLATION_PATTERN_WITH_ESCAPE: Regexp.union(       /%%/,      
                                        INTERPOLATION_PATTERN
     PATTERN_EUC:                       '[\xa1-\xfe][\xa1-\xfe]'
     PATTERN_SJIS:                      '[\x81-\x9f\xe0-\xef][\x40-\x7e\
                                        x80-\xfc]'
     PATTERN_UTF8:                      '[\xc0-\xdf][\x80-\xbf]|[\xe0-\x
                                        ef][\x80-\xbf][\x80-\xbf]'
...
-----------------
     ...
     blank?, block_scanf, bytes, bytesize, capitalize, capitalize!,
     casecmp, center, chars, chomp, chomp!, chop, chop!, column_of,
     concat, count, crypt, delete, delete!, downcase, downcase!, dump,
     each, each_byte, each_char, each_line, empty?, end_regexp,
     end_with?, ends_with?, eql?, expand_ch_hash, ext, getbyte, gsub,
     gsub!, hash, hex, include?, indent, index, initialize_copy, insert,
     inspect, intern, is_binary_data?, is_complex_yaml?, iseuc, issjis,
     isutf8, jcount, jlength, jsize, kconv, length, line_of, lines,
     ljust, lstrip, lstrip!, match, mbchar?, next, next!, oct,
     original_succ, original_succ!, parse_csv, partition, pathmap,
     pathmap_explode, pathmap_partial, pathmap_replace, replace,
     reverse, reverse!, rindex, rjust, rpartition, rstrip, rstrip!,
     scan, scanf, shellescape, shellsplit, size, slice, slice!, split,
     squeeze, squeeze!, start_with?, starts_with?, strip, strip!, sub,
     sub!, succ, succ!, sum, swapcase, swapcase!, tabto, to_blob,
     to_date_lex, to_date_time_lex, to_f, to_i, to_i_lex, to_s, to_str,
     to_sym, to_time_lex, to_xs, to_yaml, toeuc, tojis, tosjis, toutf16,
     toutf8, tr, tr!, tr_s, tr_s!, treetop_camelize, unpack, upcase,
     upcase!, upto
