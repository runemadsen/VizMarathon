...
//    if (abs(degrees(ang)%180-180) < 10) target += PI;
    ...
    s.add(new Vec2D(avg/2, radians(-26) - PI/2).toCartesian());
    for (int i = 0; i < n; i++) {
      Vec2D a = new Vec2D(avg/2, i*TWO_PI/n + radians(-5) - PI/2).toCartesian();
      Vec2D b = new Vec2D(avg/2 + thisYear[i]/2, i*TWO_PI/n - PI/2).toCartesian();
      Vec2D c = new Vec2D(avg/2, i*TWO_PI/n + radians(5) - PI/2).toCartesian();
      s.add(a).add(b).add(c);
    }
